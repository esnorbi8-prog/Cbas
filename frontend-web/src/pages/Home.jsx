...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/categorias'),
      api.get('/productos', { params: { solo_activos: true, limit: 8 } })
    ]).then(([cats, prods]) => {
  ...
  function filtrarPorCategoria(id) {
    setCatActiva(id)
    api.get('/productos', { params: { categoria_id: id, solo_activos: true, limit: 8 } })
    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh', color: '#f0f0f0', fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, maxWidth: '520px', position: 'relative', zIndex: 1, width: '100%' }}>
          <div style={{
            ...
            ⚡ FERRETERÍA #1 EN NASCA
          </div>
          <h1 style={{ fontSize: isMobile ? '30px' : '42px', fontWeight: '600', lineHeight: 1.15, marginBottom: '16px' }}>
            Todo para tu<br />
            <span style={{ color: '#F5C100' }}>obra y hogar</span>
          </h1>
          <p style={{ fontSize: '15px', color: '#aaa', lineHeight: 1.7, marginBottom: '28px' }}>
            Cemento, fierro, herramientas y más.<br />
            Paga con Yape, recoge en tienda.
          </p>
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <button onClick={() => navigate('/productos')} style={{
              padding: '13px 28px', borderRadius: '8px', fontSize: '14px', fontWeight: '600',
              background: '#F5C100', color: '#0a0a0a', border: 'none', cursor: 'pointer'
            }}>Ver catálogo →</button>
            <a href="https://wa.me/51999888777" target="_blank" rel="noreferrer" style={{
              padding: '13px 28px', borderRadius: '8px', fontSize: '14px', fontWeight: '500',
              background: 'transparent', color: '#f0f0f0', border: '0.5px solid #555',
        ...
        <div style={{
          ...
          height: isMobile ? '200px' : '210px',
          borderRadius: '14px', overflow: 'hidden', border: '2px solid #F5C100'
        }}>
          ...
          <div style={{
            ...
            padding: '5px 12px', fontSize: '11px', color: '#F5C100', fontWeight: '500'
          }}>📍 Jr. Lima 123, Nasca, Ica</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {[['📱','Pago con Yape'],['💬','Chatbot 24/7'],['🏪','Recojo en tienda'],['📦','Stock garantizado']].map(([icon, label], i) => (
          <div key={label} style={{
            ...
            borderRight: (!isMobile && i < 3) ? '1px solid #1a1a1a' : 'none',
      ...
      <div style={{ padding: isMobile ? '28px 16px 0' : '40px 32px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          {catActiva && (
            <button onClick={() => verTodosDeCategoria(catActiva)}
              style={{ fontSize: '12px', color: '#F5C100', background: 'none', border: 'none', cursor: 'pointer' }}>
              Ver todos →
            </button>
          )}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(3, 1fr)' : 'repeat(auto-fill, minmax(110px, 1fr))', gap: '8px' }}>
          {categorias.map(cat => (
            <div key={cat.id} onClick={() => filtrarPorCategoria(cat.id)} style={{
              background: catActiva === cat.id ? 'rgba(245,193,0,0.08)' : '#1a1a1a',
              border: `0.5px solid ${catActiva === cat.id ? 'rgba(245,193,0,0.5)' : '#2a2a2a'}`,
              borderRadius: '10px', padding: '14px 8px', textAlign: 'center', cursor: 'pointer'
            }}>
      ...
      {/* PRODUCTOS */}
      <div style={{ padding: isMobile ? '24px 16px 32px' : '32px 32px 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h2 style={{ fontSize: '16px', fontWeight: '500' }}>Productos destacados</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
          {productos.map(p => (
            ...
            >
              <div style={{
                background: '#111', height: isMobile ? '100px' : '130px',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '36px', borderBottom: '0.5px solid #2a2a2a', overflow: 'hidden'
              }}>
                {p.imagen_url
              ...
              <div style={{ padding: '12px' }}>
                <div style={{ fontSize: '12px', fontWeight: '500', marginBottom: '3px', lineHeight: 1.3 }}>{p.nombre}</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  <span style={{
                    ...
                    background: p.stock_actual > 5 ? 'rgba(59,109,17,0.15)' : 'rgba(226,75,74,0.12)',
                    color: p.stock_actual > 5 ? '#5a9e30' : '#E24B4A'
                  }}>{p.stock_actual > 0 ? `${p.stock_actual} disp.` : 'Sin stock'}</span>
                </div>
      ...
      <div style={{ padding: isMobile ? '0 16px 32px' : '0 32px 40px' }}>
        <div style={{
          background: 'rgba(245,193,0,0.06)', border: '0.5px solid rgba(245,193,0,0.2)',
          borderRadius: '12px', padding: isMobile ? '20px' : '24px 28px',
          display: 'flex', flexDirection: isMobile ? 'column' : 'row',
          ...
          <div>
            <div style={{ fontSize: '14px', fontWeight: '500', marginBottom: '4px' }}>
              Paga con <span style={{ color: '#F5C100' }}>Yape</span> — rápido y seguro
            </div>
            <div style={{ fontSize: '12px', color: '#666' }}>
              Transfiere al 999888777 y envíanos la captura.
            </div>
          </div>
          <button onClick={() => navigate('/productos')} style={{
